...
// 5대 리그 섹션
// ============================================================================
final majorLeaguesFixturesProvider =
    FutureProvider<Map<int, List<ApiFootballFixture>>>((ref) async {
  final service = ApiFootballService();
  final season = DateTime.now().month >= 8 ? DateTime.now().year : DateTime.now().year - 1;

  // 5대 리그 ID: EPL(39), 라리가(140), 세리에A(135), 분데스리가(78), 리그1(61)
  const leagueIds = [39, 140, 135, 78, 61];
  final result = <int, List<ApiFootballFixture>>{};

  for (final leagueId in leagueIds) {
    try {
      final fixtures = await service.getFixturesByLeague(leagueId, season);
      result[leagueId] = fixtures;
    } catch (e) {
      result[leagueId] = [];
    }
  }
...
class _MajorLeaguesSection extends ConsumerWidget {
  static const _textPrimary = Color(0xFF111827);
  static const _textSecondary = Color(0xFF6B7280);
  static const _border = Color(0xFFE5E7EB);
  ...
  static const _leagues = [
    {'id': 39, 'name': 'Premier League', 'short': 'EPL'},
    {'id': 140, 'name': 'La Liga', 'short': '라리가'},
    {'id': 135, 'name': 'Serie A', 'short': '세리에A'},
    {'id': 78, 'name': 'Bundesliga', 'short': '분데스'},
    {'id': 61, 'name': 'Ligue 1', 'short': '리그1'},
  ];

  @override
  Widget build(BuildContext context, WidgetRef ref) {
    final fixturesAsync = ref.watch(majorLeaguesFixturesProvider);
            ...
            data: (fixturesMap) {
              return Column(
                children: _leagues.map((league) {
                  ...
                  final fixtures = fixturesMap[leagueId] ?? [];
                  return _LeagueCard(
                    leagueId: leagueId,
                    leagueName: league['name'] as String,
                    shortName: league['short'] as String,
                    fixtures: fixtures,
                  );
                }).toList(),
              );
...
class _LeagueCard extends StatelessWidget {
  ...
  @override
  Widget build(BuildContext context) {
    final leagueLogo = fixtures.isNotEmpty ? fixtures.first.league.logo : null;

    return GestureDetector(
      onTap: () => context.push('/league/$leagueId/fixtures'),
      child: Container(
        margin: const EdgeInsets.only(bottom: 10),
        padding: const EdgeInsets.all(14),
        decoration: BoxDecoration(
  ...
  Widget _buildPlaceholder() {
    return Center(
      child: Icon(Icons.sports_soccer, color: _textSecondary, size: 20),
    );
  }
}
